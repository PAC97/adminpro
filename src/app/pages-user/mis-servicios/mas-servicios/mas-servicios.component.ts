...
export class MasServiciosComponent implements OnInit {
  ...
  addSer(nombre: string, descripcion: string, _id: string, img: string) {
    ...
    if (items.length > 0) {
      ...
      this.servicios.push(
        { nombre: nombre, descripcion: descripcion, _id: _id, img: img });
    ...
    if (this.servicios.length > 0) {
      ...
      this.Servicios.forEach(element => {
        ...
        if (items.length > 0) {
          ...
          this.servicios.push({ nombre: element.nombre, descripcion: element.descripcion, _id: element._id, img:element.img});
